# Final Review Round XX - {slug}

## Reviewed Artifacts
- `./docs/workflows/{slug}/idea.md`
- `./docs/workflows/{slug}/spec.md`
- `./docs/workflows/{slug}/plan.md`
- `./docs/workflows/{slug}/execution.md` when present
- [Latest implementation-review round.]
- [Implementation diff, summary, or validation evidence reviewed.]

## Prior Review Rounds
- [Optional prior round path and what changed since then.]

## Reviewer Roster
- [Reviewer persona] -> [Agent id] -> [Display name when available]

## Reviewer Synopses
- [Display name / persona]: [1-2 sentence synopsis of the reviewer's main points and tone.]

## Review Scope
- [Idea/spec/plan fidelity reviewed, implemented change considered, validation evidence kept in view, and unresolved implementation-review context checked.]

## Key Findings
- [Merged findings-first summary covering fidelity gaps, regressions, validation gaps, implementation drift, simplicity drift, or markdown-artifact gaps.]

## Meaningful Disagreements
- [Only disagreements that materially affect closure readiness.]

## Suggested Revisions
- [Concrete fix to route back to the owning stage before docs close-out.]

## Recommendation
- [Use one allowed recommendation exactly as written in this skill.]

## Outstanding Dissent
- [Any unresolved reviewer concern.]
